...
def wakeUpWordRecognition():
    ...
    print(f'LEMMY: Anytime call me ... (press Ctrl+C to exit)')
    print(f'Sample rate: {recorder.sample_rate} Hz, Frame length: {recorder.frame_length}')
    try:
        while True:
...
def listen_for_voice():
    ...
    print(f"Listening with sample rate: {cobra.sample_rate} Hz and frame length: {cobra.frame_length}")
    ...
    while True:
        ...
        if voice_activity > 0.3:
            ...
            frames.append(pcm)
            print(f"Data type at detection: {type(pcm)}, Data shape: {np.shape(pcm)}, Sample rate: {cobra.sample_rate} Hz")
            print("Voice started")
            while True:
                pcm = audio_stream.read(cobra.frame_length, exception_on_overflow=False)
                pcm = struct.unpack_from("h" * cobra.frame_length, pcm)
                ...
                if voice_activity <= 0.3:
                    if len(frames) > 0:
                        ...
                        print(f"Data type after removing background noise: {type(pcm_data)}, Shape: {pcm_data.shape}, Sample rate: {cobra.sample_rate} Hz")
                        return pcm_data
                    else:
                        frames = []
                        break
...
def speech_to_text(pcm_data):
    ...
    print(f"Data type at STT: {type(audio_data)}, Shape: {audio_data.shape}, Sample rate: 16000 Hz")  # Whisper model sample rate
