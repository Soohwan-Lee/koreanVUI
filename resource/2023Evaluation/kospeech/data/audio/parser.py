...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path: str, augment_method: int) -> Tensor:
        ...
        # 확인
        if feature is None or np.isnan(feature).any() or np.isinf(feature).any():
            logger.info("Invalid feature: {0}".format(audio_path))
            print("Invalid feature: {0}".format(audio_path))
        ...
        if self.normalize:
            ...
            if feature_std == 0:
                logger.info("Zero std deviation in feature: {0}".format(audio_path))
                print("Zero std deviation in feature: {0}".format(audio_path))
            feature /= feature_std
        ...
        if torch.isinf(feature).any():
            # 확인
            print("Audio path: {0}".format(audio_path))
            print(feature)
